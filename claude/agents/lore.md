...
2. Deduce candidate project name:
   a. Run `bash ~/.claude/hooks/path-resolve.sh`. Take `PROJECT_ROOT` from
      its output as the project root — this resolves correctly inside a
      linked worktree (Sidecar's, for instance), unlike deriving it from
      `git rev-parse --show-toplevel` directly. See `PATH_RESOLUTION.md`.
      Extract the final path component as the candidate name.
      Record `PROJECT_ROOT` as `cwd_path`.
