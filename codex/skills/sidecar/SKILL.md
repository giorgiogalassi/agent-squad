...
1. **Discover.** From the project root (the source checkout, not the
   ...
   with their parent as part of the copy. Run this via a shell command:

   ```bash
   find . \( -name .git -o -path './.codex/worktrees' \) -prune \
...
2. **Reproduce each one**, at the same relative path inside the
   ...
   - **Copy to a temp sibling, then atomically rename it into place**,
     via shell commands: `<target>.sidecar-tmp` first, `mv
     <target>.sidecar-tmp <target>` only after the copy exits 0. This is
     what makes an interrupted copy detectable and safe: the real target
     path never exists until the copy is complete, so a leftover
     `.sidecar-tmp` directory from a cancelled or crashed run is
     unambiguously partial. Before copying, remove any leftover
     `.sidecar-tmp` from a previous interrupted attempt (`rm -rf
     <target>.sidecar-tmp`) and start clean — never resume into a
     partial copy.
   - **Tiered copy strategy, cheapest first, first success wins**, each
     issued as a shell command:
...
3. **Two independent comparisons per directory. Report which one failed
   ...
   a. **Branch drift** — worktree lockfile vs source-checkout lockfile.
      ...
      branch.
      - Both present, same filename: compare content via a shell
        command — `diff -q
        <source-checkout>/<relative-path>/<lockfile>
        <worktree-path>/<relative-path>/<lockfile>`. Identical ->
        consistent, record it as such (a positive result, not just the
        absence of a warning). Different -> **mismatch**, record the
        lockfile name, the relative path, and the install command
        (table below) to run in `<worktree-path>`.
...
Inside the worktree, via a shell command:
